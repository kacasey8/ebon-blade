      ...
      for( int j = y; j < d; j += 4 ) {
        ...
        for( int i = z; i < e; i += 4 ) {
          ...
          for( int k = 0; k < n_a4; k += 4 ) {
            ...
            tempA1 = _mm_loadu_ps(asel+k0); // load the 16 values for A we will be using in this loop
            tempA2 = _mm_loadu_ps(asel+k0+m_a4);
            tempA3 = _mm_loadu_ps(asel+k2);
            tempA4 = _mm_loadu_ps(asel+k2+m_a4);
